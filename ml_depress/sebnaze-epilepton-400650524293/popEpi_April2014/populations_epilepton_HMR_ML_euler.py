...
a=1.; b=3.; c=1.; d=5.;
#---pop2
aa=8.; V1=-1.2; V2=18.; V3=12.; V4=17.4; phi=0.067;
gCa=4.; gK=8.; gL=2.; E_K=-84.; E_L=-60.; E_Ca=120.; Cm=20.
#---synapses
Vt=2.; Kd=5.; Tmax=1.; 
Ee=0.; alpha_e=1.1; beta_e=0.19; 
Ei=-80.; alpha_i=5.; beta_i=0.18;
GsEE=g_x1x1; GsEI=g_x1x2; GsIE=g_x2x1; GsII=g_x2x2; #synaptic coupling
Ce1 = CpES1; Ce2=CpES2;
...
x1 = 2.5*pb.rand(nbn1,1).ravel()-1.; y1 = 5.*pb.rand(nbn1,1).ravel()-5.;
x2 = 2.*pb.rand(nbn2,1).ravel()-1.; y2 = pb.rand(nbn2,1).ravel();
z = 0.1*pb.rand(nbn1,1).ravel()+3.45;
uE=zeros((n_e,1)).ravel(); uI=zeros((n_i,1)).ravel();
#define integration parameter if manual integration (i.e euler) 
t_end=t_stop; dt=0.05; t = arange(0,t_end,dt)
#create storage arrays
x_1s = zeros((nbn1,t_end)); x_2s = zeros((nbn2,t_end)); y_1s = zeros((nbn1,t_end)); n_s = zeros((nbn2,t_end)); z_s = zeros((nbn1,t_end));
...
for t_step in t:
    ...
    #update variables
    x1=x1+dx1*dt; x2=(V+dV*dt)/20.; y1=y1+dy1*dt; n=n+dn*dt; z=z+dz*dt; 
    uE=uE+duE*dt; uI=uI+duI*dt;
    #store
    if(pb.mod(t_step,1)==0):
        x_1s[:,t_step]=x1.ravel(); x_2s[:,t_step]=x2.ravel(); z_s[:,t_step]=z.ravel();
        y_1s[:,t_step]=y1.ravel(); n_s[:,t_step]=n.ravel(); 
        print t_step   
...
ax3.plot(t_plot, 0.2*mean(x_2s,axis=0) + 0.8*mean(x_1s,axis=0), 'purple', label='x2bar + x1bar')

...
def computeKOPs(noise2, noiseRatio, x0):
    ...
    x1_scaling = 2/(x1max-x1min)
    y1_scaling = 2/(y1max-y1min)
    x2_scaling = 2/(x2max-x2min)
    y2_scaling = 2/(y2max-y2min)
...
if __name__ == "__main__":
    ...
    computeKOPs(sys.argv[1], sys.argv[2], sys.argv[3])

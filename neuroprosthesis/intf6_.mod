: $Id: intf6.mod,v 1.80 2011/07/30 19:28:18 billl Exp $
...
static void applyEXSTDP (id0* pcell,double myspkt) {
  ...
  for(i=0;i<pcell->econvsz;i++) {//check presynaptic E cells, if they fired earlier, potentiate synapse
    ...
    ppre->wgain[idx] += inc * exp( -d / tau ); // increment the wgain of the synapse
    if(ppre->wgain[idx]<0.) ppre->wgain[idx]=0.; // check bounds of wgain
    else if(ppre->wgain[idx]>maxw) ppre->wgain[idx]=maxw;
  ...
  for(i=0;i<pcell->dvt;i++) { // check postsynaptic targets, if they fired earlier, depress the synapse
    ...
    if( (d = myspkt - pg->lastspk[poid] ) < maxplastt) {
      ...
      pcell->wgain[i] -= inc * exp( -d / tau ); // increment the wgain of the synapse
      if(pcell->wgain[i]<0.) pcell->wgain[i]=0.; // check bounds of wgain
      else if(pcell->wgain[i]>maxw) pcell->wgain[i]=maxw;
...
:** vers gives version
PROCEDURE vers () {
  printf("$Id: intf6.mod,v 1.80 2011/07/30 19:28:18 billl Exp $\n")
}
...
:** floc(x,y[,z],radius,vid,vdist,type) // find cells within distance 'r' from a location 
